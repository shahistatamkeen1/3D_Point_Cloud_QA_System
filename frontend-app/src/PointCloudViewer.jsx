...
const clearAllMeasurements = () => {
  ...
  const computeHotspots = (points, severities) => {
    ...
    critical.forEach((p) => {
      const key = `${Math.round(p.originalX / cellSize)}_${Math.round(
        p.originalY / cellSize
      )}_${Math.round(p.originalZ / cellSize)}`;
  ...
  const renderPointCloud = (
    ...
    for (let i = 0; i < pointData.length; i++) {
      ...
      if (
        ...
        if (currentFilterMode !== "all" && severity !== currentFilterMode) {
          continue;
        }
  ...
  const loadAnalysis = async (currentMode = mode, currentThreshold = threshold) => {
    try {
      setStatusMessage(
        currentMode === "raw"
          ? "Loading raw point cloud..."
          : currentMode === "comparison"
          ? "Loading comparison view..."
          : "Loading deviation heatmap..."
      );

      const response = await axios.get(`${API_BASE}/pointcloud-analysis`, {
        params: {
          mode: currentMode,
      ...
      if (!Array.isArray(pointData) || pointData.length === 0) {
        clearPointCloud();
        setStatusMessage(
          data.message || "No points found. Upload both reference and actual files if using Compare."
        );
        return;
      }
      ...
      setStatusMessage(
        currentMode === "raw"
          ? "Raw 3D point cloud loaded."
          : currentMode === "comparison"
          ? "Reference vs actual comparison loaded."
          : "3D deviation heatmap loaded."
      );
    } catch (error) {
...
savedMeasureMarkersRef.current.push(...measureMarkersRef.current);
  ...
  const handlePointHover = (event) => {
    if (
      ...
      pointsDataRef.current.length > 50000 ||
      !mountRef.current ||
      !cameraRef.current ||
  ...
  const buildAISuggestions = () => {
    ...
    const criticalPercent = pointCount
      ? (severityCounts.critical / pointCount) * 100
      : 0;

    const warningPercent = pointCount
      ? (severityCounts.warning / pointCount) * 100
      : 0;

    if (pointCount === 0) {
      ai.push("Upload or compare valid point cloud files to generate insights.");
      ...
      } else if (criticalPercent > 30) {
        ai.push("Moderate deviation detected. Several areas may require inspection.");
      } else {
        ai.push("Most compared points are within acceptable tolerance.");
      }
    } else if (mode === "deviation") {
      if (criticalPercent > 40) {
        ai.push("High deviation concentration detected in the current scan.");
      } else {
        ai.push("Deviation pattern appears manageable under the current threshold.");
      }
    } else {
      ai.push("Raw mode loaded. Switch to Deviation or Compare mode for QA insights.");
    }

    if (hotspotCount > 0) {
      ai.push(`${hotspotCount} critical hotspot area(s) detected. These represent concentrated defect zones.`);
    }

    if (maxDeviation > threshold * 2) {
      ai.push("Maximum deviation is significantly above the selected threshold.");
    }

    if (warningPercent > 10) {
      ai.push("Warning-level deviations are present and should be reviewed before they become critical.");
    }
  ...
  const buildDefectClassification = () => {
    ...
    let result = {
      type: "Low Risk",
      confidence: 70,
      reason: "Most points appear within acceptable tolerance.",
    };

    if (mode === "raw" || pointCount === 0) {
      result = {
        type: "Not Available",
        confidence: 0,
        reason: "Switch to Deviation or Compare mode to classify defects.",
      };
    } else if (
      mode === "comparison" &&
      alignmentQuality.status === "Poor Alignment" &&
      criticalPercent > 40
    ) {
      result = {
        type: "Likely Misalignment",
        confidence: 88,
        reason:
          "High critical deviation combined with poor ICP alignment suggests scan/reference registration issues.",
      };
    } else if (criticalPercent > 70 && hotspotCount <= 2) {
      result = {
        type: "Missing Geometry / Major Mismatch",
        confidence: 84,
        reason:
          "Most points are critical, but hotspots are limited, suggesting broad geometry mismatch or missing model areas.",
      };
    } else if (criticalPercent > 20 && hotspotCount >= 3) {
      result = {
        type: "Localized Surface Defect",
        confidence: 82,
        reason:
          "Critical points are concentrated into multiple hotspot zones, suggesting localized construction defects.",
      };
    } else if (warningPercent > 20 && criticalPercent < 20) {
      result = {
        type: "Scan Noise / Borderline Issues",
        confidence: 76,
        reason:
          "Warning-level deviations dominate while critical points remain low, suggesting borderline tolerance or scan noise.",
      };
    }
  ...
  const buildFixSuggestions = () => {
    ...
    if (pointCount === 0) {
      fixes.push("Upload both reference and actual files before running comparison.");
    }
    ...
    if (mode === "comparison") {
      ...
      if (hotspotCount > 0) {
        fixes.push(
          `Prioritize the largest hotspot clusters first. Start with ${
            hotspotSizes.slice(0, 3).join(", ") || "the visible hotspots"
          } critical-point clusters.`
        );
      }

      if (maxDeviation > threshold * 2) {
        fixes.push("Inspect maximum-deviation areas manually. These may represent missing geometry, scan noise, or major construction mismatch.");
      }
    }

    if (mode === "deviation") {
      if (criticalPercent > 40) {
        fixes.push("Review the red regions as likely out-of-tolerance surface areas.");
      }

      if (warningPercent > 15) {
        fixes.push("Use Warning filter to inspect borderline regions before they become critical defects.");
      }
    }

    if (filterMode !== "all") {
      fixes.push(`Current filter is '${filterMode}'. Switch back to 'all' before final reporting to include full-context results.`);
    }

    if (performanceMode !== "full") {
      fixes.push("Performance mode is limiting advanced interactions. Use smaller samples for detailed hover and hotspot inspection.");
    }

    if (fixes.length === 0) {
      fixes.push("No major corrective action suggested. Continue monitoring deviation results and verify with project tolerance requirements.");
    }
  ...
  const exportDeviationReport = () => {
    const rows = [
      ...
      [
        ...
        filterMode,
        hotspotCount,
        alignmentQuality.fitness,
        alignmentQuality.rmse,
        alignmentQuality.status,
        qualityScore.score,
        qualityScore.status,
        defectClassification.type,
        `${defectClassification.confidence}%`,
        defectClassification.reason,
      ],
      [],
      ["Severity Summary"],
    ...
    hotspotCenters.forEach((center) => {
      rows.push([
        center.id,
        center.originalX.toFixed(4),
        center.originalY.toFixed(4),
        center.originalZ.toFixed(4),
        center.size,
      ]);
    });
...
measurementHistory.forEach((m, index) => {
  ...
  const exportPDFReport = () => {
    ...
    if (rendererRef.current) {
      snapshot = rendererRef.current.domElement.toDataURL("image/png");
    }
    ...
    doc.setFontSize(18);
    doc.text("Point Cloud QA Report", 20, y);
    y += 14;
    ...
    doc.setFontSize(10);
    addWrapped(`Mode: ${mode}`);
    addWrapped(`File: ${fileName}`);
    addWrapped(`Reference: ${referenceFileName}`);
    addWrapped(`Actual: ${actualFileName}`);
    y += 4;
    ...
    doc.setFontSize(10);
    addWrapped(`Score: ${qualityScore.score}/100`);
    addWrapped(`Status: ${qualityScore.status}`);
    y += 4;

    doc.setFontSize(12);
    doc.text("Analysis Metrics", 20, y);
    y += 8;

    doc.setFontSize(10);
    addWrapped(`Points: ${pointCount}`);
    addWrapped(`Deviation Points: ${deviationCount}`);
    addWrapped(`Deviation %: ${deviationPercent}%`);
    addWrapped(`Max Deviation: ${maxDeviation}`);
    addWrapped(`Avg Deviation: ${avgDeviation}`);
    addWrapped(`Threshold: ${threshold}`);
    y += 4;

    if (mode === "comparison") {
      doc.setFontSize(12);
      doc.text("ICP Alignment Quality", 20, y);
      y += 8;

      doc.setFontSize(10);
      addWrapped(`Status: ${alignmentQuality.status}`);
      addWrapped(`Fitness: ${alignmentQuality.fitness}`);
      addWrapped(`RMSE: ${alignmentQuality.rmse}`);
      y += 4;
    }

    doc.setFontSize(12);
    doc.text("Deviation Distribution", 20, y);
    y += 8;

    doc.setFontSize(10);
    addWrapped(`Acceptable: ${severityCounts.acceptable}`);
    addWrapped(`Warning: ${severityCounts.warning}`);
    addWrapped(`Critical: ${severityCounts.critical}`);
    y += 4;

    doc.setFontSize(12);
    doc.text("AI Defect Classification", 20, y);
    y += 8;

    doc.setFontSize(10);
    addWrapped(`Type: ${defectClassification.type}`);
    addWrapped(`Confidence: ${defectClassification.confidence}%`);
    addWrapped(`Reason: ${defectClassification.reason}`);
    y += 4;

    maybeAddPage();

    doc.setFontSize(12);
    doc.text("Hotspot Summary", 20, y);
    y += 8;

    doc.setFontSize(10);
    addWrapped(`Critical Hotspots: ${hotspotCount}`);
    addWrapped(`Largest Clusters: ${hotspotSizes.length ? hotspotSizes.slice(0, 3).join(", ") : "None"}`);
    y += 4;
    ...
    doc.setFontSize(10);
    if (!suggestions.length) addWrapped("- No AI suggestions available.");
    ...
    doc.setFontSize(12);
    doc.text("Fix Recommendations", 20, y);
    y += 8;

    doc.setFontSize(10);
    if (!fixSuggestions.length) addWrapped("- No fixes required.");
    else fixSuggestions.forEach((f) => {
      maybeAddPage();
      addWrapped(`- ${f}`);
    });
  ...
  useEffect(() => {
    ...
    loadAnalysis(mode, threshold);
  }, [isSceneReady, mode, threshold, filterMode]);
...
useEffect(() => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (dragStateRef.current.panel === "left") {
        setLeftPanelPos({
      ...
      if (dragStateRef.current.panel === "right") {
        setRightPanelPos({
          x: e.clientX - dragStateRef.current.offsetX,
          y: e.clientY - dragStateRef.current.offsetY,
        });
    ...
    <div style={{ width: "100%", height: "100%", position: "relative", background: "#000", overflow: "hidden" }}>
      ...
      >
        <div
          ...
          style={{ cursor: "move", fontWeight: "bold", marginBottom: "10px", paddingBottom: "6px", borderBottom: "1px solid rgba(255,255,255,0.18)", fontSize: "15px" }}
        >
          Deviation Heatmap Viewer
        </div>

        <div style={{ marginBottom: "8px" }}>
          <div style={{ fontSize: "12px", marginBottom: "4px" }}>Single File</div>
          <input type="file" accept={ACCEPTED_FORMATS} onChange={handleFileUpload} />
        </div>

        <div style={{ marginTop: "8px" }}>
          <div style={{ fontSize: "12px", marginBottom: "4px" }}>Reference</div>
          <input type="file" accept={ACCEPTED_FORMATS} onChange={handleReferenceUpload} />
        </div>

        <div style={{ marginTop: "8px" }}>
          <div style={{ fontSize: "12px", marginBottom: "4px" }}>Actual</div>
          <input type="file" accept={ACCEPTED_FORMATS} onChange={handleActualUpload} />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "6px", marginTop: "10px" }}>
          ...
          <button onClick={() => setAutoRotate((prev) => !prev)} style={{ ...baseButton, background: autoRotate ? "#16a34a" : "#444" }}>{autoRotate ? "Auto Rotate ON" : "Auto Rotate OFF"}</button>
...
>
  Capture View
        ...
        <div style={{ ...cardStyle, lineHeight: 1.45 }}>
          ...
          {mode === "comparison" && (
            <>
              <div><strong>Reference:</strong> {referenceFileName}</div>
              <div><strong>Actual:</strong> {actualFileName}</div>
            </>
          )}
          <div><strong>Points:</strong> {pointCount}</div>
          <div><strong>Deviation Points:</strong> {deviationCount}</div>
          <div><strong>Deviation %:</strong> {deviationPercent}%</div>
          <div><strong>Max Dev:</strong> {maxDeviation}</div>
          <div><strong>Avg Dev:</strong> {avgDeviation}</div>
          <div style={{ marginTop: "8px", fontSize: "12px", color: "#aaa" }}>
            Performance Mode: <b>{performanceMode.toUpperCase()}</b>
          </div>
        </div>

        {mode !== "raw" && (
          <div style={{ marginTop: "12px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <div style={{ background: "rgba(34,197,94,0.14)", color: "#86efac", border: "1px solid rgba(34,197,94,0.25)", borderRadius: "999px", padding: "6px 10px", fontSize: "11px", fontWeight: 600 }}>
              Acceptable: {severityCounts.acceptable} ({pointCount ? ((severityCounts.acceptable / pointCount) * 100).toFixed(1) : 0}%)
            </div>
            <div style={{ background: "rgba(234,179,8,0.14)", color: "#fde047", border: "1px solid rgba(234,179,8,0.25)", borderRadius: "999px", padding: "6px 10px", fontSize: "11px", fontWeight: 600 }}>
              Warning: {severityCounts.warning} ({pointCount ? ((severityCounts.warning / pointCount) * 100).toFixed(1) : 0}%)
            </div>
            <div style={{ background: "rgba(239,68,68,0.14)", color: "#fca5a5", border: "1px solid rgba(239,68,68,0.25)", borderRadius: "999px", padding: "6px 10px", fontSize: "11px", fontWeight: 600 }}>
              Critical: {severityCounts.critical} ({pointCount ? ((severityCounts.critical / pointCount) * 100).toFixed(1) : 0}%)
            </div>
          </div>
        )}

        <div style={cardStyle}>
          <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>Selected Point</div>
          {selectedPoint ? (
            <div style={{ fontSize: "12px", color: "#d1d5db", lineHeight: 1.5 }}>
              <div><strong>Index:</strong> {selectedPoint.index}</div>
              <div><strong>X:</strong> {selectedPoint.x}</div>
              <div><strong>Y:</strong> {selectedPoint.y}</div>
              <div><strong>Z:</strong> {selectedPoint.z}</div>
              <div><strong>Deviation:</strong> {selectedPoint.deviation}</div>
              <div><strong>Status:</strong> {selectedPoint.severity}</div>
            </div>
          ) : (
            <div style={{ fontSize: "12px", color: "#9ca3af" }}>Click a point in the 3D view to inspect it.</div>
          )}
        </div>

        <div style={{ marginTop: "12px", color: "#d1d5db", fontSize: "12px", lineHeight: 1.5 }}>
          {statusMessage}
          {mode !== "raw" && <div style={{ marginTop: "4px", color: "#9ca3af" }}>Current filter: {filterMode}</div>}
        </div>
      </div>
      ...
      >
        <div onMouseDown={(e) => startPanelDrag(e, "right")} style={{ cursor: "move", fontWeight: "bold", marginBottom: "10px", paddingBottom: "6px", borderBottom: "1px solid rgba(255,255,255,0.18)", fontSize: "15px" }}>
          QA Tools & Insights
        </div>

        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>Overall Quality Score</div>
            <div style={{
              fontSize: "32px",
              fontWeight: "bold",
              color: qualityScore.score >= 90 ? "#22c55e" : qualityScore.score >= 75 ? "#84cc16" : qualityScore.score >= 55 ? "#eab308" : "#ef4444",
            }}>
              {qualityScore.score}/100
            </div>
            <div style={{ fontSize: "12px", color: "#d1d5db", marginTop: "4px" }}>
              Status: <strong>{qualityScore.status}</strong>
            </div>
          </div>
        )}
        ...
        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>Score Breakdown</div>
            <div style={{ fontSize: "12px", color: "#d1d5db", lineHeight: 1.6 }}>
              <div><strong>Deviation Impact:</strong> {Math.min(deviationPercent * 0.8, 45).toFixed(1)} penalty</div>
              <div><strong>Hotspot Impact:</strong> {Math.min(hotspotCount * 2, 20)} penalty</div>
              {mode === "comparison" && (
                <>
                  <div><strong>Alignment Boost:</strong> +{Math.min(alignmentQuality.fitness * 10, 10).toFixed(1)}</div>
                  <div><strong>Alignment Error:</strong> -{Math.min(alignmentQuality.rmse * 10, 15).toFixed(1)}</div>
                </>
              )}
            </div>
          </div>
        ...
        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>AI Defect Classification</div>
            <div style={{
              fontSize: "16px",
              fontWeight: "bold",
              color: defectClassification.type === "Low Risk" ? "#22c55e" : defectClassification.type === "Scan Noise / Borderline Issues" ? "#eab308" : "#ef4444",
            }}>
              {defectClassification.type}
            </div>
            <div style={{ fontSize: "12px", color: "#d1d5db", marginTop: "6px" }}>
              <strong>Confidence:</strong> {defectClassification.confidence}%
            </div>
        ...
        {mode === "comparison" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>ICP Alignment Quality</div>
            <div style={{ fontSize: "12px", color: "#d1d5db", lineHeight: 1.5 }}>
              <div>
                <strong>Status:</strong>{" "}
                <span style={{
                  color: alignmentQuality.status === "Good Alignment" ? "#22c55e" : alignmentQuality.status === "Moderate Alignment" ? "#eab308" : "#ef4444",
                }}>
                  {alignmentQuality.status}
                </span>
              </div>
              <div><strong>Fitness:</strong> {alignmentQuality.fitness}</div>
              <div><strong>RMSE:</strong> {alignmentQuality.rmse}</div>
            </div>
          </div>
        )}
        ...
        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>Hotspot Detection</div>
            <div style={{ fontSize: "12px", color: "#d1d5db", lineHeight: 1.5 }}>
              <div><strong>Critical Hotspots:</strong> {hotspotCount}</div>
              <div><strong>Marker Centers:</strong> {hotspotCenters.length || "None"}</div>
              <div><strong>Hotspot IDs:</strong> {hotspotCenters.length ? hotspotCenters.slice(0, 6).map((c) => `H${c.id}`).join(", ") : "None"}</div>
              <div><strong>Largest Clusters:</strong> {hotspotSizes.length ? hotspotSizes.slice(0, 3).join(", ") : "None"}</div>
            </div>
          </div>
        )}
        ...
        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>AI Suggestions</div>
            {suggestions.map((s, i) => (
              <div key={i} style={{ fontSize: "12px", color: "#d1d5db", marginBottom: "6px", lineHeight: 1.5 }}>• {s}</div>
            ))}
          </div>
        )}
        ...
        {mode !== "raw" && (
          <div style={cardStyle}>
            <div style={{ fontSize: "12px", fontWeight: "bold", marginBottom: "8px" }}>Fix Recommendations</div>
              ...
              {["all", "acceptable", "warning", "critical"].map((f) => (
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    cursor: "pointer",
                    background: filterMode === f ? (f === "critical" ? "#dc2626" : f === "warning" ? "#ca8a04" : f === "acceptable" ? "#16a34a" : "#2563eb") : "#444",
                    color: "white",
                    fontSize: "11px",
    ...
    Turn Measure Mode ON, then click two points in the 3D view.
        ...
        <div style={cardStyle}>
          ...
          <div style={{ marginTop: "8px", fontSize: "11px", color: "#9ca3af", lineHeight: 1.5 }}>
            {mode === "comparison" ? (
              <>
                <div>Green = actual point is close to reference</div>
                <div>Yellow = moderate mismatch</div>
                <div>Red = large mismatch from reference</div>
                <div>Current threshold: {threshold} → effective compare tolerance: {(threshold / 10).toFixed(2)}</div>
              </>
            ) : mode === "deviation" ? (
              <>
                <div>Green = low deviation</div>
                <div>Yellow = medium deviation</div>
                <div>Red = high deviation</div>
                <div>Current threshold: {threshold} → effective tolerance: {(threshold / 10).toFixed(2)}</div>
              </>
            ) : (
        ...
        <div style={{ marginTop: "10px", color: "#9ca3af", fontSize: "12px", lineHeight: 1.45 }}>
          <div>Drag panel header = move panel</div>
